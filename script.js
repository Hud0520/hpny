...
function updateGlobals(timeStep, lag) {
	...
	if (store.state.config.autoLaunch) {
		...
		if (autoLaunchTime <= 0) {
			const multiplier = store.state.config.finale ? 1.25 : 0.2;
			autoLaunchTime = startSequence() * multiplier;
		}
	}

	// Finale mode cycle
	finaleCountdown -= timeStep;
	if (finaleCountdown <= 0) {
		finaleCountdown = 40000;
		const currentConfig = store.state.config;
		store.setState({
			config: Object.assign({}, currentConfig, { finale: !currentConfig.finale })
		});
	}
}
